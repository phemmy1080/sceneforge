        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {filtered.map((proj) => (
            ...
            >
              ...
              {/* Meta */}
              <p className="text-[11.5px] text-white/40 mb-2">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10.5px] text-white/25">{proj.createdAt}</span>
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={(e) => { e.stopPropagation(); handleOpen(proj.id) }}
                    className="w-6 h-6 bg-[#22222F] rounded text-white/40 hover:text-white/80 text-[10px] flex items-center justify-center transition-colors"
                    title="Open"
                  >▶</button>
                  <button
                    onClick={(e) => { e.stopPropagation(); duplicateProject(proj.id) }}
                    className="w-6 h-6 bg-[#22222F] rounded text-white/40 hover:text-white/80 text-[11px] flex items-center justify-center transition-colors"
                    title="Duplicate"
                  >⊕</button>
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteProject(proj.id) }}
                    className="w-6 h-6 bg-[#22222F] rounded text-red-400/50 hover:text-red-400 text-[11px] flex items-center justify-center transition-colors"
                    title="Delete"
                  >×</button>
                </div>
              </div>
          ...
          <div
            onClick={() => document.dispatchEvent(new CustomEvent('open-new-project-modal'))}
            className="border border-dashed border-white/12 rounded-xl flex flex-col items-center justify-content-center gap-2 min-h-[120px] cursor-pointer hover:border-white/25 transition-all duration-150 items-center justify-center"
          >
            ...
            <span className="text-[12px] text-white/25">New project</span>
          </div>
        </div>
      )}
    </div>
