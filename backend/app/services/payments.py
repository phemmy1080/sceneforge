...
from app.services.geo import detect_currency, build_plans_response
...
async def get_plans(
    ...
    """Return available token plans with localized pricing (NGN for Nigeria, USD elsewhere)."""
    # Detect currency from client IP
    ...
    currency = await detect_currency(client_ip)

    plans_raw = await pricing_service.get_plans(redis)
    plans_list = build_plans_response(plans_raw, currency)

    return {"plans": plans_list, "currency": currency}
...
async def initiate_payment(
    ...
    # Get plan and detect localized currency
    client_ip = request.headers.get("x-forwarded-for") or request.headers.get("x-real-ip") or (request.client.host if request.client else None)
    currency = await detect_currency(client_ip)

    raw_plans = await pricing_service.get_plans(redis)
    plans_list = build_plans_response(raw_plans, currency)
